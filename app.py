...
def get_vector_store(text_chunks):
    if not text_chunks:
        ...
        return None
    # Use os.getenv to get the API key directly from the environment variable
    embeddings = OpenAIEmbeddings(openai_api_key=os.getenv("OPEN_AI_APIKEY"))
    vectorstore = FAISS.from_texts(texts=text_chunks, embedding=embeddings)
    return vectorstore

def get_conversation_chain(vector_store):
    llm = ChatOpenAI(openai_api_key=os.getenv("OPEN_AI_APIKEY"))
